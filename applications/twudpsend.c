...
int parse_user_params(char * file_name)
{
    ...
    for(i = 0; i < cJSON_GetArraySize(json_file); i++)
    {
        ...
        user_params.server_ip = tw_convert_ip_str_to_dec(
                cJSON_GetObjectItem(subitem, "ServerIP")->valuestring);
        user_params.server_port = tw_convert_str_to_int(
                cJSON_GetObjectItem(subitem, "ServerPort")->valuestring,
                strlen(cJSON_GetObjectItem(subitem, "ServerPort")->valuestring));
        user_params.payload_size = tw_convert_str_to_int(
                cJSON_GetObjectItem(subitem, "Payload")->valuestring,
                strlen(cJSON_GetObjectItem(subitem, "Payload")->valuestring));
        user_params.test_runtime = tw_convert_str_to_int(
                cJSON_GetObjectItem(subitem, "testRuntime")->valuestring,
                strlen(cJSON_GetObjectItem(subitem, "testRuntime")->valuestring));
        user_params.pps_limit = tw_convert_str_to_int(
                cJSON_GetObjectItem(subitem, "ppsLimit")->valuestring,
...
void pkt_tx(tw_tx_t * handle)
{
    ...
    if((curr_time_cycle - prev_stats_calc) > ppsdelay)
    {
        ...
        if((tw_stats.pkts_tx < PacketLimit || PacketLimit == 0)
                && (tw_stats.secs_passed < user_params.test_runtime || user_params.test_runtime == 0))
        {
            if(dst_eth_addr == NULL)
            {
                struct arp_table * temp_arp_entry = tw_search_arp_table(
                        tw_be_to_cpu_32(user_params.server_ip));
            ...
            else
            {
                ...
                udp->dgram_len = tw_cpu_to_be_16(
                        tx_buf->size - sizeof(struct ether_hdr) - sizeof(struct ipv4_hdr));
                udp->dgram_cksum = 0;
