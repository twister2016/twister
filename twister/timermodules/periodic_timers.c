...
int tw_update_periodic_timers(uint64_t time_limit) {							//Time limit will be in millisecs
	curr_periodic_freq = tw_get_current_timer_cycles();
	if (tw_is_timer_expired(curr_periodic_freq, prev_periodic_freq, one_msec, time_limit)) {
		rte_timer_manage();
...
int tw_stats_callback_timer_init(void) {
    ...
    int tem= tw_create_periodic_timer(&stat_timer, stats_time_period, rte_lcore_id(), (rte_timer_cb_t) tw_print_global_stats, NULL);
    if (tem!=0)
