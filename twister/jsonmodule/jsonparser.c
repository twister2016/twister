    ...
    for(port_id = 0; port_id < total_eth_ports; port_id++)
    {
        ...
        {			//For each port_num
            ...
            if(curr_port_name_str != NULL)
            {
                ...
                for(j = 0; j < cJSON_GetArraySize(ip_addrs); j++)
                {			//For each start_ip_addr and range
                    cJSON * subdictip = cJSON_GetArrayItem(ip_addrs, j);
                    port_info[port_id].start_ip_addr = tw_convert_ip_str_to_dec(
                            cJSON_GetObjectItem(subdictip, "ip_address")->valuestring);
                    port_info[port_id].gateway_ip = tw_convert_ip_str_to_dec(
                            cJSON_GetObjectItem(subdictip, "gateway_ip")->valuestring);
                    port_info[port_id].subnet_mask = tw_convert_ip_str_to_dec(
                            cJSON_GetObjectItem(subdictip, "subnet_mask")->valuestring);
                }
            }
            else
            {
                printf("NULL VALUE FROM JSON....get_port_conf_json_vals.................\n");
            }
