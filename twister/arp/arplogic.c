...
struct ether_addr broadcastmac = { .addr_bytes[1] = 0xff, .addr_bytes[2] = 0xff, .addr_bytes[3] = 0xff,
        .addr_bytes[4] = 0xff, .addr_bytes[5] = 0xff, .addr_bytes[0] = 0xff, };

struct ether_addr querycastmac = { .addr_bytes[1] = 0x00, .addr_bytes[2] = 0x00, .addr_bytes[3] = 0x00,
        .addr_bytes[4] = 0x00, .addr_bytes[5] = 0x00, .addr_bytes[0] = 0x00, };
...
int tw_parse_arp(struct rte_mbuf * pkt, uint8_t port_id)
{
    ...
    if(rte_be_to_cpu_16(arp_pkt->arp_op) == ARP_OP_REQUEST)
    {
        ...
        if(rte_be_to_cpu_32(arp_pkt->arp_data.arp_tip) == port_info[port_id].start_ip_addr)
        { //&& (port_info[port_id].flags & REPLY_ARP)) {
            if(tw_search_arp_table(arp_pkt->arp_data.arp_sip) == NULL)
            { //Save ARP entry for local use also
...
int tw_add_arp_entry(uint32_t ip_to_add, struct ether_addr mac_to_add, uint8_t port_id)
{
    ...
    {
        arp_table_ptr = rte_malloc("struct arp_table", sizeof(struct arp_table),
                                   RTE_CACHE_LINE_SIZE);
    ...
    else
    {
        while (arp_table_ptr->next != NULL)
            arp_table_ptr = arp_table_ptr->next;
        arp_table_ptr->next = rte_malloc("struct arp_table", sizeof(struct arp_table),
                                         RTE_CACHE_LINE_SIZE);
        arp_table_ptr = arp_table_ptr->next;
    }
    if(arp_table_ptr == NULL)
...
void tw_print_arp_table(void)
{
    ...
    while (temp_arp_entry != NULL)
    {
        ...
        printf("\n|    %s ", inet_ntoa(ip_addr));
        printf("  |     %02X:%02X:%02X:%02X:%02X:%02X    |", temp_arp_entry->eth_mac.addr_bytes[0],
               temp_arp_entry->eth_mac.addr_bytes[1], temp_arp_entry->eth_mac.addr_bytes[2],
               temp_arp_entry->eth_mac.addr_bytes[3], temp_arp_entry->eth_mac.addr_bytes[4],
