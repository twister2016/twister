...
int search_arp_table(uint32_t);
int add_arp_entry(uint32_t, struct ether_addr);
/******************************************************************************************/
...
int	send_arp_reply(struct ether_hdr * eth, uint8_t port_id) {
	...
	add_pkt_to_tx_queue((rte_mbuf *) eth, port_id);				//--!TODO implement add_pkt_to_tx_queue
	return 0;
}	

int process_arp_reply(struct ether_hdr * eth, uint8_t port_id) {
	struct arp_hdr * arp_pkt = (struct arp_ipv4 *) (eth+1);
	if(search_arp_table(arp_pkt->arp_data.arp_sip) == NULL)	{	//if ARP entry doesn't exist. One MAC can have multiple IPs associated
		if(add_arp_entry(arp_pkt->arp_data.arp_sip, arp_pkt->arp_data.arp_sha))
			return -1;
	}
	rte_pktmbuf_free((rte_mbuf *) eth);
	return 0;
...
int add_arp_entry(uint32_t ip_to_add, struct ether_addr mac_to_add) {
	struct arp_table * arp_table_ptr = arp_table_root;
	while(arp_table_ptr != NULL)
			arp_table_ptr = arp_table_ptr->next;
	arp_table_ptr = rte_malloc(struct arp_table, sizeof(struct arp_table), 64);
